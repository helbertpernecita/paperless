...
<section class="content">
    <div class="card text-sm">
        <div class="card-header">
          <a href="{{ route('positions.create') }}" class="btn ml-2 btn-primary btn-sm py-0" data-bs-toggle="modal" data-bs-target="#modal-add-position"> Add position </a>
          @if ($errors->any())
        ...
        <div class="card-body table">
          <table id="positionTable" class="table table-bordered table-hover text-sm">
            <thead>
            <tr>
              <th>Count</th>
              <th>Name</th>
            ...
            <tbody>
              @foreach ($positions as $key=>$row)
                <tr>
                  ...
                  <td>
                    <div class="btn-group">
                      <a href="{{ route('positions.edit', $row->id) }}" class="btn  btn-primary btn-sm py-0" data-bs-toggle="modal" data-bs-target="#modal-update-position">Edit </a>
                      @if ($row->active == 1)
                        <form action="{{ route('positions.status', $row->id) }}" method="post">
                          @csrf
                          @method('PATCH')
...
<script>
    $('#modal-update-position').bind("show.bs.modal", function(e){
        var link = $(e.relatedTarget);
        $(this).find(".modal-body").load(link.attr("href"));
    })

    $('#modal-add-position').bind("show.bs.modal", function(e){
        var link = $(e.relatedTarget);
        $(this).find(".modal-body").load(link.attr("href"));
    })
</script>

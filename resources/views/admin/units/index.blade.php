...
<div class="content-header">
    <div class="container-fluid">
        ...
        <div class="col-sm-6">
            ...
            <li class="breadcrumb-item active">Units of Measurements</li>
            </ol>
        </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
...
<section class="content">
    <div class="card text-sm">
        <div class="card-header">
          <a href="{{ route('units.create') }}" class="btn ml-2 btn-primary btn-sm py-0" data-bs-toggle="modal" data-bs-target="#modal-add-unit"> Add unit </a>
          @if ($errors->any())
        ...
        <div class="card-body table">
          <table id="unitTable" class="table table-bordered table-hover text-sm">
            <thead>
            <tr>
              <th>Count</th>
              <th>Name</th>
            ...
            <tbody>
              @foreach ($units as $key=>$row)
                <tr>
                  ...
                  <td>
                    <div class="btn-group">
                      <a href="{{ route('units.edit', $row->id) }}" class="btn  btn-primary btn-sm py-0" data-bs-toggle="modal" data-bs-target="#modal-update-unit">Edit </a>
                      @if ($row->active == 1)
                        <form action="{{ route('units.status', $row->id) }}" method="post">
                          @csrf
                          @method('PATCH')
...
<script>
    $('#modal-update-unit').bind("show.bs.modal", function(e){
        var link = $(e.relatedTarget);
        $(this).find(".modal-body").load(link.attr("href"));
    })

    $('#modal-add-unit').bind("show.bs.modal", function(e){
        var link = $(e.relatedTarget);
        $(this).find(".modal-body").load(link.attr("href"));
    })
</script>
